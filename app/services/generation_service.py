...
class GenerationService:
    ...
    def __init__(self, max_workers: int = 2, queue_size: int = 100):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="generation-worker"
        )
        ...
        logger.info(
            f"Generation service initialized with {max_workers} workers, queue size {queue_size}"
        )
    ...
    def start(self):
        ...
        self.worker_thread = threading.Thread(
            target=self._worker_loop, name="generation-service-worker", daemon=True
        )
    ...
    def _process_request(self, request: GenerationRequest):
        ...
        logger.info(
            f"Processing generation request {task_id} with priority {request.priority.name}"
        )
        ...
        try:
            ...
            future = self.executor.submit(
                self._run_generation, request.request_data, request.progress_callback
            )
    ...
    def _run_generation(self, request_data: Any, progress_callback: Callable) -> Any:
        ...
        try:
            ...
            loop.run_until_complete(progress_callback("zipping", 20, "Creating ZIP file..."))
            ...
            loop.run_until_complete(
                progress_callback(
                    "complete", 100, "Generation complete! ZIP file ready for download."
                )
            )
...
def get_generation_service() -> GenerationService:
    ...
    if generation_service is None:
        ...
        generation_service = GenerationService(
            max_workers=settings.generation_workers, queue_size=settings.generation_queue_size
        )
